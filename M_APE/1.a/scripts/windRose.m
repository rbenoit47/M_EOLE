...
%windRose   Wind rose of direction and intensity
% 
%   Syntax:
%      [HANDLES,DATA] = windRose(D,U,VARARGIN)
%
%   Inputs:
%      D   Directions
%      U   Velocities
...
%
%      figure
%      windRose(D,V,'iflip',1)
%
%      figure
%      windRose(D,V,'ci',[1 2 7],'dtype','meteo')
%
%      % place it on a previous axes:
%      ax=axes;
%      plot(lon,lat)
%      windRose(D,V,'ax',[ax x y 1/3])
%
...
legStr='';
%RB
f=figure('visible','on');  %RB toggle to on
cmap=jet;  %RB déclenche une figure!
close(f)  %RB
...
if isempty(Ag)
  % gen Ag:
  f=figure('visible','off');  %RB toggle to on
  plot(F); axis tight;
  yl=get(gca,'ytick');
  close(f)
...
if legType==1 % contimuous.
  for j=1:length(Ag)-1
    lab=num2str(Ag(j));
%RB    if j==1 & hasL & IncHiLow
	if j==1 && hasL && IncHiLow  %RB
      lab='';
    end
    y1=y0+h;
...
elseif legType==2 % separated boxes.
  for j=1:length(Ag)-1
    ...
    if j==1 && hasL && IncHiLow % RB  & hasL & IncHiLow
      lab=['<',num2str(Ag(2))];
    end
    if j==length(Ag)-1 && hasH && IncHiLow   %RB & hasH & IncHiLow
      lab=['>=',num2str(Ag(j))];
    end
    y1=y0+h;
